...
Discipline document for the `.agents/skills/` surface in this repo. The skill content and the categories spec live here. The validator and link checker run as pre-commit hooks stamped from the shared agentic-os hook suite.
...
3. Stage with `git add` and commit. The pre-commit hooks run automatically:
   ...
   - `ward-lockdown` - audit-log and permission-boundary hooks generated by ward.
...
When the canonical scripts in agentic-os change, re-run the shared hook rollout from the appropriate control repo to re-stamp local copies into every consumer repo. Re-run the suite to confirm nothing broke before pushing.
