...
Companion to [`../SKILL.md`](../SKILL.md). Structural reference for the `.agents/skills/` surface in this repo. Rules below are enforced by the shared hook suite against [`.agents/skills/categories.yaml`](../../categories.yaml).
...
* `ward-lockdown` - audit-log trailer and permission boundary. Generated by ward.
* `skill-conventions` - structure, size, prefix taxonomy. From agentic-os.
