...
Usage:
    ...
    python3 scripts/validate_skills.py <name> ...   # validate only the named skills
    python3 scripts/validate_skills.py --report-only  # exit 0 even on failures (drift report)
...
SPEC_PATH = next((p for p in _SPEC_CANDIDATES if p.exists()), _SPEC_CANDIDATES[-1])
...
# Backticked references like `kai-foo`, `daily-bar`, `ops-investigation-baz`.
# Built dynamically from the prefixes in categories.yaml.
...
class Spec:
    ...
    @property
    def enforce_em_dash_check(self) -> bool:
        return bool(self.raw.get("enforce_em_dash_check", False))
...
def strip_for_prose_scan(text: str) -> str:
    """Mask inline code, URLs, quoted strings, link targets, and fenced code
    so prose-grepping doesn't false-positive on legitimate quoted content."""
    ...
    text = MD_LINK_RE.sub(
        lambda m: m.group("text") + " " * (len(m.group(0)) - len(m.group("text"))), text
    )
    out = []
    in_fence = False
...
def check_em_dashes(md_path: Path, body: str, report: Report) -> None:
    masked = strip_for_prose_scan(body)
    for m in EM_DASH_RE.finditer(masked):
...
def run_global_checks(
    ...
    for entry in iter_skill_dirs():
        ...
        if spec.enforce_em_dash_check:
            check_em_dashes(skill_md, body, report)
